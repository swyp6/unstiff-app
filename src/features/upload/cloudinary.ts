...
// 재현이 간헐적인 서명/업로드 실패를 디버깅할 수 있게 개발 빌드에서만 남긴다.
// AxiosError 전체(config.headers.Authorization, 요청 body)나 응답 본문 전문은
// 콘솔에 남기지 않고 status/code/message만 남긴다 — ImageUploadError의 cause에
// 담긴 AxiosError도 같은 이유로 그대로 출력하지 않는다.
